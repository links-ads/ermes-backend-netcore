...
{
    public class MapRequestTests: ErmesTestBase
    {
        private const string TYPE_MAP_REQUEST = "MapRequest";
        ...
        [Fact]
        public async Task Should_Get_MapRequests()
        {
            var token = await GetToken(USERNAME_OM);
            token.ShouldNotBeNull();
            var client = GetApiClient(token);
            Uri uri = new Uri(client.BaseAddress, "MapRequests/GetMapRequests" + BASE_QUERY_PARAMS);
            HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Get, uri);
            var responseValue = await SendHttpRequest(request, client);
            responseValue.ShouldNotBeNull();
            var apiOutput = JsonConvert.DeserializeObject<DTResult<MapRequestDto>>(responseValue);
            ...
            var apiOutput2 = JsonConvert.DeserializeObject<DTResult<MapRequestDto>>(responseValue2);
            ...
            var apiOutput3 = JsonConvert.DeserializeObject<DTResult<MapRequestDto>>(responseValue3);
            ...
            var apiOutput4 = JsonConvert.DeserializeObject<DTResult<MapRequestDto>>(responseValue4);
